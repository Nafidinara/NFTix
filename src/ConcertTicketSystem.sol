...
contract ConcertTicketSystem is Ownable, Pausable, ReentrancyGuard {
    ...
    function getAllConcerts(bool isActive) public view returns (Concert[] memory) {
        uint256 count = 0;
    ...
    function addCart(uint256 _concertId, uint256 _ticketClassIndex, uint256 _quantity) public {
    ...
    function addConcert(
        ...
        require(_startBuy >= block.timestamp, "Start buy time must be in the future");
        ...
        address nftAddress = nftFactory.createNFT(nftName, _symbol, baseIPFSHash);
        concertNFTs[newConcertId] = nftAddress;

        emit ConcertAdded(newConcertId, _concertName, _artistName, _venue, _date);
    }
